...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["image-convert"];
  return {
...
export default async function ImageConvertPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "image-convert")} />
      ...
      <ImageConverter ui={tool.ui} common={dict.common} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="image-convert" dict={dict} />
    </article>
  );
}
