...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["image-compress"];
  return {
...
export default async function VideoConvertPage({
  ...
  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "image-compress")} />
      ...
      <ImageCompressor ui={tool.ui} common={dict.common} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="image-compress" dict={dict} />
    </article>
  );
}
