...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["remove-bg"];
  return {
...
export default async function RemoveBgPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const dict = getDictionary(locale);
  const tool = dict.tools["remove-bg"];

  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "remove-bg")} />
      ...
      <BackgroundRemover ui={tool.ui} common={dict.common} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="remove-bg" dict={dict} />
    </article>
  );
}
