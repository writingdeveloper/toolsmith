...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools.upscale;
  return {
...
export default async function UpscalePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const dict = getDictionary(locale);
  const tool = dict.tools.upscale;

  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "upscale")} />
      ...
      <Upscaler ui={tool.ui} common={dict.common} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="upscale" dict={dict} />
    </article>
  );
}
