...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["pdf-organize"];
  return {
...
export default async function PdfOrganizePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const dict = getDictionary(locale);
  const tool = dict.tools["pdf-organize"];

  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "pdf-organize")} />
      ...
      <PdfOrganizer ui={tool.ui} common={dict.common} errors={dict.pdfErrors} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="pdf-organize" dict={dict} />
    </article>
  );
}
