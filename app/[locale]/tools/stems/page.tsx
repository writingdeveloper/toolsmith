...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools.stems;
  return {
...
export default async function StemsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const dict = getDictionary(locale);
  const tool = dict.tools.stems;

  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "stems")} />
      ...
      <StemSplitter ui={tool.ui} common={dict.common} />
      ...
      <RelatedTools locale={locale} slug="stems" dict={dict} />
    </article>
  );
}
