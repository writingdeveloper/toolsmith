...
const PATH = "/tools/video-compress";
...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["video-compress"];
  return {
...
export default async function VideoCompressPage({
  ...
  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "video-compress")} />
      ...
      <VideoCompressor ui={tool.ui} common={dict.common} errors={dict.mediaErrors} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="video-compress" dict={dict} />
    </article>
  );
}
