...
export async function generateMetadata({
  ...
  const tool = getDictionary(locale).tools["image-resize"];
  return {
...
export default async function VideoConvertPage({
  ...
  return (
    <article className="space-y-10">
      <JsonLd data={toolJsonLd(locale, dict, "image-resize")} />
      ...
      <ImageResizer ui={tool.ui} common={dict.common} chain={chainCopy(locale, dict)} />
      ...
      <RelatedTools locale={locale} slug="image-resize" dict={dict} />
    </article>
  );
}
