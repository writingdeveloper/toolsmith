...
/*
 * **`lib/text/document-text.ts` 가 갖는다.** 글 속 개인정보 찾기(`/lab/pii`)가 똑같은
 * 일을 하므로 2026-07-28 에 갈라냈다 — 속성값 안의 `>` 와 UTF-16 의 NUL 은 실물
 * 파일로만 드러난 것이라, 베껴 두면 언젠가 한 곳만 고쳐진다.
 *
 * 스펙이 부르는 이름은 그대로 남긴다.
 */
export { ACCEPTED_EXTENSIONS, extractHtml } from "@/lib/text/document-text";
...
export async function readDocument(name: string, bytes: ArrayBuffer): Promise<LoadedDocument> {
  try {
    return { text: await readDocumentText(name, bytes) };
  } catch (error) {
    if (error instanceof DocumentTextError) throw new SummarizeError(error.code);
