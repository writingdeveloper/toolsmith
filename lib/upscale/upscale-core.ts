 ...
 * CPU 에서 128×128 타일 하나에 9.7초였다 — 512×512 사진이면 160초다. WebGPU 없이는
 * 못 쓰고, 테스트 Chromium 에는 WebGPU 가 아예 없어(`navigator.gpu` 부재) 스위트가
 * 검증할 수도 없다. 검증 못 하는 경로만 있는 도구는 이 저장소가 낼 물건이 아니다.
 * `realesr-general-x4v3` 은 같은 기계에서 512×512 에 16.5초 — **60배 빠르다.**
