...
test.describe("못 미더운 결과를 말한다", () => {
  test.beforeEach(async ({ page }) => {
    /*
     * **위 "브라우저" 묶음과 똑같이 모델을 돌리는데 시간 제한만 빠져 있었다.**
     * 단독으로는 3초에 끝나므로 오래 초록색이었지만, 4 워커가 함께 도는 날에는 기본
     * 30초를 넘겨 실패했다(2026-07-28 에 국내·배포본에서 한 번씩). 도구는 멀쩡했다 —
     * 잘못된 제한이 만든 실패는 정보가 아니라 잡음이다.
     */
    test.setTimeout(240_000);
    await page.goto("/ko/tools/remove-bg");
  });
