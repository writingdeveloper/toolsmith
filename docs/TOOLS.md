...
**테스트 브라우저에 WebGPU 가 없다(실측).** Playwright Chromium 은 `--enable-unsafe-webgpu`,
`--use-angle=swiftshader`, `--enable-features=Vulkan` 어느 조합에서도 `navigator.gpu` 가
아예 없다. 즉 **WebGPU 전용 도구는 스위트가 한 줄도 검증하지 못한다.** 이것이 모델 선택의
가장 무거운 제약이 됐다 — 우리 QA 규칙("결과 blob 을 실제로 디코드해서 검증한다")을
지킬 수 없는 도구는 낼 물건이 아니다.
