...
### 0. `/lab` + 글 속 개인정보 찾기 — **바로 짜면 된다 (2026-07-28 검증 완료)**

사용자가 Lab 첫 항목으로 고른 것이다. 확인은 끝났으니 재검증하지 말 것.

- 모델 `openai/privacy-filter` — **Apache-2.0**, 다운로드 50만. 글에서 이름·주소·번호를
  찾는 **토큰 분류** 모델이다(이미지가 아니다). 128k 문맥, 총 1.5B / 활성 50M.
- 실행 **`pipeline("token-classification", "openai/privacy-filter",
  { device: "webgpu", dtype: "q4" })`**, `@huggingface/transformers@4.2.0` (jsDelivr).
  **요약 도구와 같은 버전이다** — 자막 번역(3.7.6)과 통일하지 말 것.
- 최소 받을 양 **874MB**(q4). 우리 최대치(자막 번역 632MB)를 넘어서 Tier 2 가 아니라
  Lab 이다. **누르기 전에 숫자로 말하는 게이트**가 이 층의 존재 이유다.
- 요약이 `needs: "webgpu"` 를 쓴 이유(내려갈 곳이 없다)가 여기도 그대로 적용된다.
- **`/lab` 은 색인되어야 한다**(백링크가 목적). 링크하지 않는 것은 Tier 1·2 도구
  페이지에서다 — `docs/TOOLS.md` 의 Tier 3 절 참고.
