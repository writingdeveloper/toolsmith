...
2. 색인 진행 상황 — **2026-07-29 실측.** 사이트맵을 다시 제출하니 그 자리에서 재읽기가
   ...
   **색인 요청 할당량은 "하루 리셋" 이 아니다 (2026-07-29 실측 — 어제 적은 것을 정정).**
   어제 여섯 번째에서 `Quota Exceeded` 가 났고, **오늘은 첫 건이 통과하자마자 막혔다.**
   하루가 지났다고 여섯이 다시 열리지 않는다 — 롤링이다.
   **손으로 넣는 것에 기대지 말 것**: 사이트맵 재제출 한 번이 217개를 한꺼번에 다시
   물리고 그쪽에는 할당량이 없다. 손 요청은 새 글 한둘을 밀어 넣는 용도로만 쓴다.
   오늘 들어간 것은 `/en/guides/why-pdf-wont-open` 하나.
   **남은 것: `/en` 의 `csv-vs-excel` · `can-ai-summarize`.**
