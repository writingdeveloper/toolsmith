...
2. 색인 진행 상황 확인 — 사이트맵 발견 페이지가 **187**(루트 1 + 6 × (홈 + 도구 21
   + 글 목록 + 글 8))로 늘어야 한다. 2026-07-28 시점에 **163** 이었고(오늘 배포분 24는
   아직 안 읽힘) 그 자리에서 사이트맵을 **다시 제출해 재읽기를 걸어 두었다.**
   Pages·실적 리포트는 아직 "처리 중" 이다.

   **색인 요청은 하루 할당량이 있다 (2026-07-28 실측).** 여섯 번째 제출에서
   `Quota Exceeded` 가 났다. 그래서 **`/en` 신규 글 넷만 요청됐다** —
   `how-background-removal-works`(요청 몇 분 만에 **색인 완료**),
   `does-upscaling-add-detail`·`srt-vs-vtt`(크롤됨, 판정 대기),
   `what-are-stems`(대기열).
   **남은 것: `/en` 의 `what-is-heic` · `image-formats` · `why-pdf-is-large` ·
   `mov-vs-mp4`.** 다른 언어판은 hreflang 과 사이트맵으로 따라간다 — 48개를
   손으로 넣을 일이 아니다.
...
4. **설명 글을 더한다.** 여덟을 냈다 — Tier 1 넷(`what-is-heic` · `image-formats` ·
   `why-pdf-is-large` · `mov-vs-mp4`)과 Tier 2 넷(`how-background-removal-works` ·
   `does-upscaling-add-detail` · `srt-vs-vtt` · `what-are-stems`).
   틀은 끝났으므로 **글만 쓰면 된다** — 슬러그를 `lib/guides/registry.ts` 에 더하고
   `lib/guides/{locale}.ts` 여섯을 채우면 사이트맵 · hreflang · 스키마 · 도구 페이지
   링크가 전부 저절로 따라온다.
   **모델 도구 글은 성격이 다르다** — 형식 글은 "무엇이 다른가" 로 끝나지만 이쪽은
   **"못 하는 것" 이 본론**이다(지어내는 것 · 하나만 찾는 것 · 추정인 것). 규칙 3 이
   가장 크게 걸리는 자리다.
   아직 글이 없는 도구: **PDF 병합·분할·회전, OCR, CSV 쿼리, 컷아웃, 요약.**
   다만 다음 순서는 2번의 검색 데이터가 나온 뒤에 정하는 편이 낫다 — 지금 여덟은
   전부 데이터 없이 고른 것이다.
