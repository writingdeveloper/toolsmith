...
+ 루트 언어 선택. 전부 정적. Playwright **290종**. 두 프로젝트로 돈다 — 기본
...
- **스펙에서 `innerText()` 같은 호출은 기본이 "무한정 대기" 다 (2026-07-27).**
  ...
  진단용 코드에는 반드시 `{ timeout: … }` 을 준다. 같은 실수로 두 번 데였고, 두 번 다
...
**실물 파일 쓸이에서 지금까지 13건이 나왔다** — 자막 번역 4, PDF 계열 3(2026-07-26),
요약 3, OCR 1 · 이미지 압축 1 · 배경 제거 1(2026-07-27).
