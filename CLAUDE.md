...
1. **Tier 2 는 여섯 칸이 찼다.** 남은 것은 **#21 요약** 하나다.
   ...
   뻔했다. 근거는 `docs/TOOLS.md` 의 "자막 번역 — 하마터면 라이브러리 버전 때문에".
2. 색인 진행 상황 확인 — 사이트맵 발견 페이지가 **120**(6 × (홈 + 도구 19))으로 늘어야
   한다. 2026-07-26 기준 78 이었고 Pages·실적 리포트는 아직 "처리 중" 이다.
3. **검색 데이터가 잡히면 그때부터는 그것으로 다음 도구를 정한다.** 지금까지 19개는
   전부 데이터 없이 순서를 정해 만든 것이다.
