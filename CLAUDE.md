...
4. **설명 글은 열하나다.** Tier 1 넷(`what-is-heic` · `image-formats` ·
   ...
   **글에는 아직 제 공유 카드가 없다 (2026-07-29 확인).** `ogImageFor` 가
   `/tools/<slug>` 만 보므로 **글 66페이지와 Lab 12페이지가 전부 기본 `/og.png`** 를
   쓴다. 도구 21곳에는 전용 카드가 있는데 **정작 링크를 받으라고 만든 글에는 없다** —
   코드에서 지금 가장 뚜렷하게 비어 있는 자리다.
