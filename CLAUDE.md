...
+ 글 4) = 162 페이지 + 루트 언어 선택. 전부 정적. Playwright **370종**. 두 프로젝트로 돈다 — 기본
...
- `lib/i18n/dictionaries/en.ts` — 사전의 **타입 원본**. 여기에 키를 더하면 나머지 5개가
  타입 에러로 드러난다.
- `lib/guides/registry.ts` — **도구가 아닌 페이지.** 설명 글 넷(`/guides/…`)이 여기 있다.
  ...
  `docs/TOOLS.md` 의 "도구가 아닌 페이지가 처음 생겼다".
...
2. 색인 진행 상황 확인 — 사이트맵 발견 페이지가 **163**(루트 1 + 6 × (홈 + 도구 21
   + 글 목록 + 글 4))로 늘어야 한다. 2026-07-26 기준 78 이었고 Pages·실적 리포트는
   아직 "처리 중" 이다.
   **설명 글 넷을 색인 요청해 둘 것** — 도구와 달리 이쪽은 검색량이 큰 정보성 질문을
...
4. **설명 글을 더한다.** 넷을 냈고(`what-is-heic` · `image-formats` · `why-pdf-is-large`
   · `mov-vs-mp4`), 틀은 끝났으므로 **다음부터는 글만 쓰면 된다** — 슬러그를
   `lib/guides/registry.ts` 에 더하고 `lib/guides/{locale}.ts` 여섯을 채우면 사이트맵 ·
   hreflang · 스키마 · 도구 페이지 링크가 전부 저절로 따라온다.
   비어 있는 자리: Tier 2 도구(배경 제거 · 업스케일 · 자막 · 스템)를 다룬 글이 하나도
   없다. **가장 차별화된 도구들인데 설명 글이 없다.** 다만 순서는 2번의 검색 데이터가
   나온 뒤에 정하는 편이 낫다 — 지금 넷도 데이터 없이 고른 것이다.
