...
+ 글 11 + Lab 목록 + Lab 도구 1) = 216 페이지 + 루트 언어 선택.
전부 정적. Playwright **407종**. 두 프로젝트로 돈다 — 기본
...
2. 색인 진행 상황 — **2026-07-29 실측.** 사이트맵을 다시 제출하니 그 자리에서 재읽기가
   돌아 발견 페이지가 **187 → 217** 로 맞았다(루트 1 + 6 × (홈 + 도구 21 + 글 목록 +
   글 11 + Lab 목록 + Lab 도구 1)). Pages·Links 리포트는 여전히 "처리 중" 이다.

   **첫 검색 데이터가 잡혔다 (2026-07-29).** 노출 2 · 클릭 0 · 평균 게재순위 1.
   페이지는 넷이다 — `/en` · `/ko` · `/en/tools/ocr` · `/en/tools/data-query`.
   **질의는 아직 안 보인다**(표본이 구글의 비공개 임계값 아래라 "No data").
   그러니 3번의 "데이터로 정한다" 는 **아직 못 한다** — 노출이 자릿수로 늘어야 질의가 열린다.
   ...
   **색인 요청 할당량은 "하루 한 번 리셋" 이 아니다 (2026-07-29 실측 — 어제 적은 것을
   ...
   그래도 **손으로 넣는 것에 기대지 말 것**: 사이트맵 재제출 한 번이 217개를 한꺼번에
...
3. **검색 데이터로 다음을 정하는 것은 아직 이르다.** 노출 2건으로는 아무것도 못 고른다.
   질의가 열릴 때까지 기다린다 — 그 전에 도구나 글을 더 고르는 것은 지금까지 스물한 번
   해 온 것과 똑같이 **감으로 고르는 것**이다.
4. **설명 글은 열하나다.** Tier 1 넷(`what-is-heic` · `image-formats` ·
   `why-pdf-is-large` · `mov-vs-mp4`), Tier 2 넷(`how-background-removal-works` ·
   `does-upscaling-add-detail` · `srt-vs-vtt` · `what-are-stems`), 그리고 **세어서
   고른 셋**(`why-pdf-wont-open` · `csv-vs-excel` · `can-ai-summarize`).
   **이제 21개 도구가 전부 글 하나 이상을 가리킨다** — 비는 자리를 세는 방법은 다 썼다.
   다음 묶음은 3번의 질의 데이터를 기다린다.
